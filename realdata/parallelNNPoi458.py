...
if __name__ == '__main__':
    ...
    df188["Y"] = df188["unitvisitnumber"] - 1
    ...
    # Create binary target Y
    df458["Y"] = df458["unitvisitnumber"] - 1
    ...
    sample_set_train = SampleSet(0.9, X_train_tensor, y_train_tensor, 1, module='Poisson')
    sample_set_train.get_sub_samples_with_validation(B)

    # Create and save testing SampleSet
    sample_set_test = SampleSet(0.9, X_test_tensor, y_test_tensor, 1, module='Poisson')

    sample_set_train.save(f'sampleset/transfer/sampleset188{p}poisson{B}_trainfolder.pth')
    sample_set_test.save(f'sampleset/transfer/sampleset458{p}poisson{B}_testfolder.pth')
    ...
    hatf_B, sd_f, sd_f_c = train_and_predict_NN_parallel(sample_set_train, X_test, mode="Poisson",n_jobs=23)

    out_path = os.path.join(results_folder, f'resultspoi{p}NN{B}_folder.pth')
    torch.save({
    ...
    # print(plot_lift_curve(hatf_B,y_test_tensor))
