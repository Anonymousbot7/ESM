...
def process_folder(i):
    ...
    hatf_B, sd_f, sd_f_c = train_and_predict_NN_parallel(sample_train, X_test, mode="Poisson",n_jobs=20)

    out_path = os.path.join(results_folder, f'resultspoi{p}NN{B}_folder{i}.pth')
    torch.save({
        'y_test': y_test,
        'hatf_B': hatf_B,
        'sd_f'  : sd_f,
