...
######Constant Area#######
    ...
    network = NeuralNetwork(p, 128, 64, dropout_rate=0.1)
    r        = int(n ** 0.65)
    ss = SampleSet(n, p, f_1, module=GLM_name)
    ss.get_sub_samples_with_validation(B, r)
    train_samples, validation_samples, selection_counts = ss.subtrain, ss.subval, ss.counts

    X_sub, Y_sub, _ = train_samples[0]
    X_val, Y_val, _ = validation_samples[0]

        
    train_dataset = TensorDataset(X_sub, Y_sub)
    train_loader = DataLoader(train_dataset, batch_size=r, shuffle=True)
    

    network = NeuralNetwork(p, 128, 64, dropout_rate=0.1)

    # Warm up
    train_network(network, train_loader,mode=GLM_name, num_epochs=500, learning_rate=0.1,weight_decay=0.02,tol=0.0001,patience=7)


    for index_e in [0.65,0.7,0.75,0.8,0.85,0.9,0.95]:
        ...
        print(f'the time of $\gamma$={index_e}:')
        tic()
        train_network(network, train_loader,mode=GLM_name, num_epochs=500, learning_rate=0.1,weight_decay=0.02,tol=0.0001,patience=7)
        toc()
