...
class SampleSet:
    ...
    def get_sub_samples_split(self, B):

        indices = torch.arange(self.n)
        self.B = B


        shuffled = indices[torch.randperm(self.n)]

        base_size = self.n // B
        remainder = self.n % B


        split_sizes = [base_size + (1 if i < remainder else 0) for i in range(B)]


        splits = torch.split(shuffled, split_sizes)
        ...
        for selected_indices in splits:
            ...
            sub_samples.append((X_sub, Y_sub, selected_indices))

        self.subtrain = sub_samples   
        return sub_samples
...
def train_multiple_networks(sample_set, input_size, hidden_size1, hidden_size2,mode="Bernoulli",batchsize=64, dropout_rate=0.1, num_epochs=100, learning_rate=0.01,weight_decay=0.05,tol=0.001,patience=3):
    ...
    for i, (train_data) in enumerate(train_samples):
...
def run_one_repeat(rep_id, n, B, p, GLM_name, f_1, xtest):
    ...
    pred_min, _ = torch.min(Af, dim=1)
    pred_max, _ = torch.max(Af, dim=1)
...
######Constant Area#######
    ...
    fn0 = f"{folder}/Hulc/{GLM_name}uppern{n}p{p}.csv"
    fn1 = f"{folder}/Hulc/{GLM_name}lowern{n}p{p}.csv"
