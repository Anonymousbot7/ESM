...
class SampleSet:
    ...
    def get_bootstrap_sample(self,nbp):
   


        indices = torch.randint(low=0, high=n, size=(nbp,), dtype=torch.long)
...
def train_multiple_networks(sample_set, input_size, hidden_size1, hidden_size2,mode="Bernoulli",batchsize=64, dropout_rate=0.1, num_epochs=100, learning_rate=0.01,weight_decay=0.05,tol=0.001,patience=3):
    ...
    for i in range(B):
        nbp = random.randint(max(1, n - 50), n)
        X_sub, Y_sub= sample_set.get_bootstrap_sample(nbp)
...
if __name__ == "__main__":
    ...
    fn0 = f"{folder}/Ensemble/{GLM_name}En{n}p{p}.csv"
    fn1 = f"{folder}/Ensemble/{GLM_name}stdn{n}p{p}.csv"
    fn2 = f"{folder}/Ensemble/{GLM_name}lowern{n}p{p}.csv"
    fn3 = f"{folder}/Ensemble/{GLM_name}uppern{n}p{p}.csv"
