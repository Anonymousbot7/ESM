    ...
    for i, (train_data, val_data) in enumerate(zip(train_samples, validation_samples)):
        ...
        with torch.no_grad():
            if mode=="Bernoulli":
                ...
                true_lambda=torch.log(1+torch.exp(f_2(X_val)))
                estimated_lambda=torch.exp(logits)
                print(f" network {i+1}: {torch.max(true_lambda),torch.min(true_lambda)}%")

                print(torch.mean(abs(true_lambda-estimated_lambda)),torch.max(abs(true_lambda-estimated_lambda)))
...
def run_one_repeat(rep_id, n, r, B, p, GLM_name, f_2, xtest):
    # 每个 repeat 训练 B 个网络，最后做一次 ensemble 预测
    ss = SampleSet(n, p, f_2, module=GLM_name)
...
######Constant Area#######
    ...
    results = Parallel(n_jobs=20)(
        delayed(run_one_repeat)(i, n, r, B, p, GLM_name, f_2, xtest)
    ...
    fn0 = f"{folder}/{GLM_name}fBf2n{n}p{p}B{B}r{r}.csv"
    fn1 = f"{folder}/{GLM_name}sdf2nn{n}p{p}B{B}r{r}.csv"
    fn2 = f"{folder}/{GLM_name}sdcrtf2nn{n}p{p}B{B}r{r}.csv"
