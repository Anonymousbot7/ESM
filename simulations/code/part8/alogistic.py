...
class NeuralNetwork(nn.Module):
    def __init__(self, input_size, L, hidden_dim=30, dropout_rate=0.1):
...
def train_multiple_networks(sample_set, input_size, mode="Bernoulli",batchsize=64, dropout_rate=0.1, num_epochs=100, learning_rate=0.01,weight_decay=0.05,tol=0.0001,patience=7):
    ...
    for i, (train_data, val_data) in enumerate(zip(train_samples, validation_samples)):
        ...
        network = NeuralNetwork(input_size, L=L, dropout_rate=dropout_rate)
...
######Constant Area#######
    ...
    folder = 'resultspart8'  
    xtest    = torch.load(f"{folder}/xtest10.pt")  
    ...
    fn0 = f"{folder}/{GLM_name}fBf1n{n}p{p}B{B}r{r}L{L}.csv"
    fn1 = f"{folder}/{GLM_name}sdf1nn{n}p{p}B{B}r{r}L{L}.csv"
    fn2 = f"{folder}/{GLM_name}sdcrtf1nn{n}p{p}B{B}r{r}L{L}.csv"
