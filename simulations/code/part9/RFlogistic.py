...
class SampleSet:
    def __init__(self, n, p,f_X,module='Bernoulli', mean=0, std=1,trials=None):
        ...
        X_main = torch.normal(0.0, 1.0, size=(n, 3))
        X_noise = torch.normal(0.0, 0.5, size=(n, p - 3))
...
# Usage:
# hatf_B, sd_raw, sd_corr = train_and_predict_RF(sample_set, X_test, mode="Bernoulli")
...
def run_one_repeat(rep_id, n, r, B, p, GLM_name, f_1, xtest):
    ...
    Bf=train_and_predict_RF(ss,xtest, mode=GLM_name)
...
######Constant Area#######
    ...
    fn0 = f"{folder}/{GLM_name}fBf1RFn{n}p{p}B{B}r{r}.csv"
    fn1 = f"{folder}/{GLM_name}sdf1RF{n}p{p}B{B}r{r}.csv"
    fn2 = f"{folder}/{GLM_name}sdcrtf1RF{n}p{p}B{B}r{r}.csv"
