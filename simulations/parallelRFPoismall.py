...
def process_folder(i):
    B=3000
    p=10
    sample_set_folder = 'samplesetpoismall/'  # Folder where sample sets are stored
    results_folder = 'resultssmallpoi/'  # Folder to save results
    train_path = os.path.join(sample_set_folder, f'sampleset{p}poisson{B}_trainfolder{i}.pth')
    test_path  = os.path.join(sample_set_folder, f'sampleset{p}poisson{B}_testfolder{i}.pth')
    ...
    hatf_B, sd_f, sd_f_c = train_and_predict_RF(sample_train, X_test, mode="Poisson")

    out_path = os.path.join(results_folder, f'results{p}RF{B}_folder{i}.pth')
    torch.save({
        'y_test': y_test,
        'hatf_B': hatf_B,
        'sd_f'  : sd_f,
